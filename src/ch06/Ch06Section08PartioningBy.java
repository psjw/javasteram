...
public class Ch06Section08PartioningBy {
    public static void main(String[] args) {
        ...
        Map<Boolean, List<User>> userPartions = users.stream()
                .collect(Collectors.partitioningBy(user -> user.getFriendUserIds().size() > 5));

        EmailService emailService = new EmailService();
        for(User user : userPartions.get(true)){
            emailService.senPlayWithFriendsEmail(user);
        }

        for(User user : userPartions.get(false)){
